...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Xauzinho @${num.split('@')[0]}🖕😉 no meio do cu dele 😉😘`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '📌minha puta pera to fazendo 😘',
				success: '📌 ai corno 〽️',
				...
				only: {
					...
					ownerG: '[🤬] Este comando só pode ser usado pelo grupo proprietário!(ninem/Netomeker😘) 〽️',
			...
			const ownerNumber = ["553488648308@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '553488648308@s.whatsapp.net'
                        /******ApiKey Input******/
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('シ︎༒︎༒︎〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 ninem BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, 'シ︎༒︎༒︎〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 ninem BOT 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					reply('シ︎༒︎༒︎〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 ninem BOT 〙')
					break
				case 'send':
					var pc = body.slice(6)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'S㋛︎シ︎✔︎', text) // ur cods
					}, 0)
                     break
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
              ...
              if (!isUser) return reply(mess.only.daftarB)
                random = `${Math.floor(Math.random() * 100)}`
               hasil = `PORCERTAGEM : *${body.slice(1)}*\n\nVOCÊ É: : *${random}%* gay... já desconfiavaシ︎`
              reply(hasil)
                break
	    case 'kapankah':
               client.updatePresence(from, Presence.composing) 
			...
			case 'closegc':
				...
				case 'animehug':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = aguardar fetchJson('https://tobz-api.herokuapp.com/api/hug&apikey=BotWeA', {method: 'get'})
                                        if (!isUser) return reply(mess.only.daftarB)
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
				...
				case 'toimg':
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('📌 responder vídeo hum 〽️')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('〽️ Falha ao converter vídeo para mp3 📌')
						buffer = fs.readFileSync(ran)
